    ...
    public function getFormElements(string $operation): array
    {
        return [
            (new Element\Text('search_query'))
                ->setOptions([
                    'label' => 'Search query',
                    'info' => 'Custom text to search for to find collections to render'
                ]),
            // @todo re-enable
//            (new Element\Checkbox('search_fallback'))
//                ->setOptions([
//                    'label' => 'Fill remaining',
//                    'info' => 'If there is not enough search results, this will use any available collections to fill the space.',
//                ])
//                ->setValue(true),
