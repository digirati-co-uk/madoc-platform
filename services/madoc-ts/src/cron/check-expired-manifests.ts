...
export async function checkExpiredManifests(context: ApplicationContext, fireDate: Date) {
  ...
  for (const project of runningProjects) {
    ...
    const shortExpiryTimeMins = !isNaN(config.shortExpiryTime) ? Number(config.shortExpiryTime) : 10; // configured value or 10 minutes
    const shortTermExpiry = new Date(Date.now() - shortExpiryTimeMins * 60 * 1000);

    const longExpiryTimeMins = !isNaN(config.longExpiryTime) ? Number(config.shortExpiryTime) : 24 * 60; // configured value or 1 day..
