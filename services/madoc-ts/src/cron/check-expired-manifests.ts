...
export async function checkExpiredManifests(context: Context, fireDate: Date) {
  ...
  for (const project of runningProjects) {
    ...
    const shortExpiryTimeMins = Number(config.shortExpiryTime) || 10; // configured value or 10 minutes
    const shortTermExpiry = new Date(Date.now() - shortExpiryTimeMins * 60 * 1000);

    const longExpiryTimeMins = Number(config.longExpiryTime) || 24 * 60; // configured value or 1 day..
