...
export const RootLoader: UniversalComponent<RootLoaderType> = createUniversalComponent<RootLoaderType>(
  () => {
    ...
    const configuredAccent = site.config?.accentColor || siteTheme?.theme?.custom?.accent?.primary;
    const accentColor =
      typeof configuredAccent === 'string' && /^#[0-9a-f]{6}$/i.test(configuredAccent) ? configuredAccent : '#4265e9';
    ...
    return (
      <ConfigProvider project={data ? data.project : undefined} navigation={data ? data.navigation : undefined}>
        <Helmet>
          ...
          <style>{`:root { --madoc-accent: ${accentColor}; --madoc-accent-text: ${makeColorAccessible(accentColor)}; }`}</style>
          <style>{`
